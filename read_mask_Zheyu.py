...
def read_mask(simg,xml_file,output_dir, Good_Bad_dir=None, match_file=False):
        ...
        if (len(regions) < 2):
            ...
            for j in range(len(regions)):
                ...
                if match_file:
                    ...
                    filename =  '%s-x-ROI_%d-x-%s-x-*.png' %  (os.path.basename(xml_file).replace('.xml', ''), j, clss_name)
    ...
    new_xml_file = os.path.join(output_dir, os.path.basename(xml_file))

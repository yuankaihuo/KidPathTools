...
def ValidateFormats(argFormat, argName, errors):
    if argFormat == 'xywh':
        ...
        errors.append(
            'argument %s: invalid value. It must be either \'xywh\' or \'xyrb\' or \'xyr\'' % argName)
...
def getBoundingCircles(directory,
                     isGT,
                     bbFormat,
                     coordType,
                     allBoundingCircles=None,
                     allClasses=None,
                     imgSize=(0, 0)):
    """Read txt files containing bounding boxes (ground truth and detections)."""
    if allBoundingCircles is None:
        allBoundingCircles = BoundingCircles()
    if allClasses is None:
        allClasses = []
    # Read ground truths
    os.chdir(directory)
        ...
        for line in fh1:
            ...
            if isGT:
                ...
                bb = BoundingCircle(
                    ...
                    confidence,
                    format=bbFormat)
            allBoundingCircles.addBoundingCircle(bb)
            if idClass not in allClasses:
                allClasses.append(idClass)
        fh1.close()
    return allBoundingCircles, allClasses
...
if gtFormat == BBFormat.XYR:
    allBoundingCircles, allClasses = getBoundingCircles(
        gtFolder, True, gtFormat, gtCoordType, imgSize=imgSize)
    # Get detected boxes
    allBoundingCircles, allClasses = getBoundingCircles(
        detFolder, False, detFormat, detCoordType, allBoundingCircles, allClasses, imgSize=imgSize)
    allClasses.sort()

    evaluator = Evaluator_Circle()
    acc_AP = 0
    validClasses = 0

    # Plot Precision x Recall curve
    detections = evaluator.PlotPrecisionRecallCurve(
        allBoundingCircles,  # Object containing all bounding boxes (ground truths and detections)
